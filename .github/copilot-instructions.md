...
│   ├── js/                 # JS files (created during build, sourced from assets/)
...
📝 **Note:** The `site_generator.py` expects files in `static/css/` and `static/js/` directories. Ensure source files from `assets/` are available at these locations before generation, or update the generator paths accordingly.
...
4. **Frontend changes go in `assets/`** - Edit CSS/JS source files, never `static/index.html`
